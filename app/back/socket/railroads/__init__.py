...
def connect_railroads(app):
    ...
    def get_railroads(query={}, filter={"_id": 0}, sort=[("properties.id", 1)], limit=0, detail="micro"):

        emit("new_railroads", find_railroads(query, filter, [tuple(item) for item in sort], limit, detail))
