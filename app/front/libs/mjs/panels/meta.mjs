export function metaUpdate(code) {
  ...
  let r = plot.r.find(item => item.year == data.plot["year"]).value
  let x = plot.x.find(item => item.year == data.plot["year"]).value
  let y = plot.y.find(item => item.year == data.plot["year"]).value
  let z = plot.z.find(item => item.year == data.plot["year"]).value
