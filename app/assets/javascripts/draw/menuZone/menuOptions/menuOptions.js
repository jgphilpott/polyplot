function drawMenuOptions(menu) {

  app.view.canvas.menu.options = [{
                                    name: "Home",
  ...
  app.view.canvas.menu.lock = {
    ...
    visible: true,
    color: app.view.theme.grayScale[3]
  };

  var options = app.view.canvas.menu.options;
  var lock = app.view.canvas.menu.lock;
  ...
  for (var i = 0; i < options.length; i++) {
    if (options[i].visible) {
      ...
      options[i].color = app.view.theme.colors[iconColor];
      iconColor += 1;

      menu.append("rect")
          .data([options[i]])
          .attr("class", "menu-icon-background layer-two")
          .attr("id", options[i].name.toLowerCase() + "-icon-background")
          .attr("width", iconSize)
      ...
      menu.append("path")
          ...
          .attr("id", options[i].name.toLowerCase() + "-icon")
          .attr("transform", "translate(" + iconSpaceingX + ", " + iconSpaceingY + ")")
          .attr("d", options[i].icon)
          .attr("fill", app.view.theme.grayScale[2]);
  ...
  if (lock.visible) {
    ...
    menu.append("rect")
        ...
        .attr("height", iconSize)
        .attr("x", iconSpaceingX)
        .attr("y", app.view.canvas.menu.height - (iconSize * 1.5))
        .attr("fill", app.view.theme.grayScale[1]);

    menu.append("path")
        .data([lock])
        .attr("class", "lock menu-icon layer-two")
        .attr("id", lock.names[0].toLowerCase() + "-icon")
        ...
        .attr("transform", "translate(" + iconSpaceingX + ", " + (app.view.canvas.menu.height - (iconSize * 1.5)) + ")")
        .attr("d", lock.icons[0])
        .attr("fill", app.view.theme.grayScale[2]);
    ...
    menu.append("path")
        .data([lock])
        .attr("class", "lock menu-icon layer-two")
        .attr("id", lock.names[1].toLowerCase() + "-icon")
        .attr("visibility", function () {
          if (lock.locked) {
            return "hidden";
          } else {
  ...
  $(".menu").mouseover(function() {
    if (app.view.canvas.menu.width === menuWidth) {
      for (var i = 0; i < options.length; i++) {
        if (options[i].visible) {
          $("#" + options[i].name.toLowerCase() + "-icon").css({"fill": options[i].color});
        };
      };
      $(".lock").css({"fill": lock.color});
    };
  }).mouseout(function() {
    if (app.view.canvas.menu.width === menuWidth) {
      for (var i = 0; i < options.length; i++) {
        if (options[i].visible) {
          $("#" + options[i].name.toLowerCase() + "-icon").css({"fill": "" + app.view.theme.grayScale[2] + ""});
        };
      };
      $(".lock").css({"fill": "" + app.view.theme.grayScale[2] + ""});
    };
  });
